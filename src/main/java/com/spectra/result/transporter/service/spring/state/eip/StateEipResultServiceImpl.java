package com.spectra.result.transporter.service.spring.state.eip;
...
import java.util.*;
@Slf4j
public class StateEipResultServiceImpl extends SpringServiceImpl implements StateEipResultService {
	//private Logger log = Logger.getLogger(StateEipResultServiceImpl.class);
						...
						if(repositoryService != null){
							...
							if(writerContext != null){
								...
								try{
									...
									String stateListStr = this.configService.getString("state.list");
									String[] stateListArray = stateListStr.split(",");
									List<String> stateList = Arrays.asList(stateListArray);
									log.debug("testExecuteStrategy(): stateList: " + (stateList == null ? "NULL" : stateList.toString()));
									...
									}else if(procType.equals("MUGSI")){
										dtoList = repositoryService.getMugsiResults(paramMap);
									}
									...
									Boolean processedAllStateDemo = Boolean.FALSE;
									if((dtoList != null) && (dtoList.size() > 0)){
										...
										if(dtoListMap != null){
											...
											for(Iterator<Map.Entry<String, List<RepositoryResultDto>>> it = entrySet.iterator(); it.hasNext();){
												...
												List<RepositoryResultDto> mapDtoList = entry.getValue();
												if((key != null) && key.length() > 0){
													processedDemo = execStrategies(key, mapDtoList, true, procType, paramMap, repositoryService, conversionContext, writerContext);
													if(processedDemo.equals(Boolean.FALSE)){
														StringBuilder errBuilder = new StringBuilder();
														errBuilder.append("process(): processedDemo: ").append(processedDemo.toString());
														errBuilder.append(" procType: ").append(procType);
														errBuilder.append(" state: ").append(state);
														errBuilder.append(" key: ").append(key);
														errBuilder.append(" conversionCtx: ").append(conversionCtx);
										...
										if(dtoList != null){
											String key = null;
											processedAllStateDemo = execStrategies(key, dtoList, true, procType, paramMap, repositoryService, conversionContext, writerContext);
											if(processedAllStateDemo.equals(Boolean.FALSE)){
												StringBuilder errBuilder = new StringBuilder();
												errBuilder.append("process(): processedAllStateDemo: ").append(processedAllStateDemo.toString());
												errBuilder.append(" procType: ").append(procType);
												errBuilder.append(" state: ").append(state);
												errBuilder.append(" key: ").append(key == null ? "NULL" : key);
												errBuilder.append(" conversionCtx: ").append(conversionCtx);
												errBuilder.append(" writerCtx: ").append(writerCtx);
												log.error(errBuilder.toString());
											}
