...
@Slf4j
public class EipPoiFileWriterStrategy implements PoiWriterStrategy {
	//private Logger log = Logger.getLogger(EipPoiFileWriterStrategy.class);
			...
			if(this.configService != null){
				...
				if(winCmdExecutorService != null){
					try{
						...
						String stateDropFolder = this.configService.getString("folder.state.drop");
						String allStateDropFolder = this.configService.getString("folder.all.state.drop");
						String stateArchiveFolder = this.configService.getString("folder.state.archive");
						String allStateArchiveFolder = this.configService.getString("folder.all.state.archive");
						...
						log.debug("write(): stateDropFolder: " + (stateDropFolder == null ? "NULL" : stateDropFolder));
						log.debug("write(): allStateDropFolder: " + (allStateDropFolder == null ? "NULL" : allStateDropFolder));
						log.debug("write(): stateArchiveFolder: " + (stateArchiveFolder == null ? "NULL" : stateArchiveFolder));
						log.debug("write(): allStateArchiveFolder: " + (allStateArchiveFolder == null ? "NULL" : allStateArchiveFolder));
						...
						if(loc != null){
							if(loc.indexOf(".") != -1){
								...
								stateLoc = loc;
								dropFolder = stateDropFolder;
								archiveFolder = stateArchiveFolder;
							}
						}else{
							dropFolder = allStateDropFolder;
							archiveFolder = allStateArchiveFolder;
						}
						...
						//drop
						StringBuilder dateFolderBuilder = new StringBuilder();
						if(runFrequency.equals("daily")){
							//dropBuilder.append(year).append(month).append(day).append("\\");
							dateFolderBuilder.append(year).append(month).append(day);
						}else if(runFrequency.equals("monthly")){
							//dropBuilder.append((currMo == 1 ? prevMonthYear : year)).append(prevMonth.toString()).append("\\");
							dateFolderBuilder.append((currMo == 1 ? prevMonthYear : year)).append(prevMonth.toString());
						}
						dropFolder = MessageFormat.format(dropFolder, new String[]{ state, stateLoc, dateFolderBuilder.toString()});
						...
						*/
						...
						//archive
						archiveFolder = MessageFormat.format(archiveFolder, new String[]{ state, stateLoc, dateFolderBuilder.toString()});
						...
						*/
						log.debug("write(): archBuilder: " + (archBuilder == null ? "NULL" : archBuilder.toString()));
