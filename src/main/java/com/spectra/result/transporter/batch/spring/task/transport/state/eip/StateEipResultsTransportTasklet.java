package com.spectra.result.transporter.batch.spring.task.transport.state.eip;
...
@Slf4j
public class StateEipResultsTransportTasklet extends ResultSchedulerTask {
	///private Logger log = Logger.getLogger(StateEipResultsTransportTasklet.class);
	...
	public RepeatStatus execute(StepContribution contr, ChunkContext chunk) throws Exception {
		...
		if((ConditionChecker.checkNotNull(this.configService)) &&
			...
			StateEipResultService stateResultService = (StateEipResultService)this.resultSchedulerServiceFactory.getContextService(StateEipResultService.class.getSimpleName());
