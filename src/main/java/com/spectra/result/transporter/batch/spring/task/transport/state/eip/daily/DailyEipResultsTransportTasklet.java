package com.spectra.result.transporter.batch.spring.task.transport.state.eip.daily;
...
@Slf4j
public class DailyEipResultsTransportTasklet extends ResultSchedulerTask {
	//private Logger log = Logger.getLogger(DailyEipResultsTransportTasklet.class);
	...
	public RepeatStatus execute(StepContribution contr, ChunkContext chunk) throws Exception {
		...
		if((ConditionChecker.checkNotNull(this.configService)) &&
			...
			DailyEipResultService stateResultService = (DailyEipResultService)this.resultSchedulerServiceFactory.getContextService(DailyEipResultService.class.getSimpleName());
