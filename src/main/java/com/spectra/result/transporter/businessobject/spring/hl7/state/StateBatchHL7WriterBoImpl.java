...
import com.spectra.result.transporter.businessobject.spring.hl7.HL7WriterConstants;
...
@Slf4j
public class StateBatchHL7WriterBoImpl implements StateHL7WriterBo {
	//private Logger log = Logger.getLogger(StateBatchHL7WriterBoImpl.class);
...
*/						
				...
				log.error(String.valueOf(ce.getMessage()));
				ce.printStackTrace();
			}
		}//if
	...
	public String toHL7String(Map<String, List<PatientRecord>> listMap) throws BusinessException {
		...
		if((listMap != null) && (this.configService != null)){
			...
			String hapiCustomPackage = null;
			try{
				fileSendingFacility = this.configService.getString("fhs.4");
				fileReceivingApplication = this.configService.getString("fhs.5");
				fileReceivingFacility = this.configService.getString("fhs.6");
				sendingApplication = this.configService.getString("msh.3");
				sendingFacilityId = this.configService.getString("msh.4.2");
				sendingFacilityIdType = this.configService.getString("msh.4.3");
				hapiCustomPackage = this.configService.getString("hapi.custom.package");
				
				log.debug("toHL7String(): fileSendingFacility: " + (fileSendingFacility == null ? "NULL" : fileSendingFacility));
				log.debug("toHL7String(): fileReceivingApplication: " + (fileReceivingApplication == null ? "NULL" : fileReceivingApplication));
				log.debug("toHL7String(): fileReceivingFacility: " + (fileReceivingFacility == null ? "NULL" : fileReceivingFacility));
				log.debug("toHL7String(): sendingApplication: " + (sendingApplication == null ? "NULL" : sendingApplication));
				log.debug("toHL7String(): sendingFacilityId: " + (sendingFacilityId == null ? "NULL" : sendingFacilityId));
				log.debug("toHL7String(): sendingFacilityIdType: " + (sendingFacilityIdType == null ? "NULL" : sendingFacilityIdType));
			}catch(ConfigException ce){
			...
			//FHS|||Spectra East|ELRS\S\2.16.840.1.113883.3.1299.5.1.6.1\S\ISO|NJDOH\S\2.16.840.1.113883.3.1299\S\ISO
			//BHS|||Spectra East|ELRS\S\2.16.840.1.113883.3.1299.5.1.6.1\S\ISO|NJDOH\S\2.16.840.1.113883.3.1299\S\ISO
