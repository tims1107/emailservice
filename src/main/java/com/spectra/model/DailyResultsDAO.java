...
public class DailyResultsDAO {
    ...
    public DailyResultsDAO(IDao dao) {
...
//            }
    ...
    public void getTests(String sql){

        _getTests(sql,"IL");
    }

    private String _createTerser(MICRO_RESULT pat_result) throws Exception {
        ...
        Message hapiMsg = p.parse(repo.ncmsg);
...
//
    ...
    private String createTerser(MICRO_RESULT pat_result){
        try {
            return _createTerser(pat_result);
        } catch (Exception e) {
    ...
    private void _getTests (String sql,String state) {
        ...
        try {
            ...
            List<String> patlist = list.stream()
                    .limit(2)
                    .map(t -> createTerser(t))
                    .collect(Collectors.toList());

            String filename = repo.getFileName(state);

            writeTerser(new StringBuffer().append(String.format(repo.fhs,repo.newDate)),filename);

            patlist.stream()
                    .forEach(t -> writeTerser(new StringBuffer().append(t),filename));

            long patcnt = patlist.stream().count();

            writeTerser(new StringBuffer().append("BTS|" + patcnt + "\rFTS|1\r"),filename);
    ...
    public LOINC_CODE getMicroOrganismFilter(String sql){
        return _getMicroOrganismFilter(sql);
    }

    public SNOMED_MASTER getSnomedCode(String sql){
        return _getSnomedCode(sql);
    }

    private SNOMED_MASTER _getSnomedCode (String sql) {

        ResultSet rst = null;
        ResultSetMetaData rsmd = null;


        Map<String, SNOMED_MASTER> lfacilities = new HashMap<>();

        QueryRunner run = new QueryRunner();

        ResultSetHandler<List<SNOMED_MASTER>> h = new BeanListHandler<SNOMED_MASTER>(SNOMED_MASTER.class);

        try {
            List<SNOMED_MASTER> list = run.query(con, sql, h);

            SNOMED_MASTER sm = list.stream()
                    .findFirst().get();
    ...
    private LOINC_CODE _getMicroOrganismFilter (String sql) {

        ResultSet rst = null;
        ResultSetMetaData rsmd = null;


        Map<String, LOINC_CODE> lfacilities = new HashMap<>();

        QueryRunner run = new QueryRunner();

        ResultSetHandler<List<LOINC_CODE>> h = new BeanListHandler<LOINC_CODE>(LOINC_CODE.class);

        try {
            List<LOINC_CODE> list = run.query(con, sql, h);

            LOINC_CODE lc = list.stream()
                    .findFirst().get();
