...
import com.spectra.asr.businessobject.hl7.v251.ILHL7Bo;
...
//import com.spectra.scorpion.framework.exception.BusinessException;
@Slf4j
public class ILHL7GeneratorStrategyImpl extends StateHL7GeneratorStrategyImpl {
	//private Logger log = Logger.getLogger(ILHL7GeneratorStrategyImpl.class);
			...
			if(this.generatorDto != null){
				...
				if(this.targetList == null){
					...
					if(targetListStr != null){
						...
						log.info("targetListArray {}",targetListArray);
						Arrays.stream(targetListArray)
								.forEach(targetList::add);

						log.info(String.valueOf(targetList));


//						System.exit(0); //Generator
...
//						}
				...
				if(this.targetList != null){
					...
					targetDtoList.stream()
							.forEach(t -> {
								log.warn("generate(): targetDtoList: target: order number: " + (t == null ? "NULL" : t.getOrderNumber()));
								log.warn("generate(): targetDtoList: target: facility number: " + (t == null ? "NULL" : t.getFacilityId()));
								log.warn("generate(): targetDtoList: target: state: " + (t == null ? "NULL" : t.getPatientAccountState()));
								log.warn("generate(): targetDtoList: target: county: " + (t == null ? "NULL" : t.getPatientAccountCounty()));
								log.warn("generate(): targetDtoList: target: pat name: " + (t == null ? "NULL" : t.getPatientLastName()));
							});
...
//					}
				...
				List<PatientRecord> patientRecordList = patRecListMap.entrySet().stream()
						.map(t -> t.getValue())
						.flatMap(Collection::stream)
						.collect(Collectors.toList());

				patientRecordList.stream()
						.forEach(System.out::println);
				...
				ILHL7Bo bo = AsrBoFactory.getILHL7v251Bo();
				HL7Dto hl7Dto = null;
				if(bo != null){
					try{
