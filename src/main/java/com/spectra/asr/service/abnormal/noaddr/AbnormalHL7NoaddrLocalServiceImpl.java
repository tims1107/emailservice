...
@Slf4j
public class AbnormalHL7NoaddrLocalServiceImpl implements AbnormalNoaddrLocalService {
	//private Logger log = Logger.getLogger(AbnormalHL7NoaddrLocalServiceImpl.class);
			...
			if(noaddrDtoList != null){
				...
				for(Map.Entry<String, List<StateResultDto>> naEntry : naEntrySet){
					...
					if((stdList != null) && (stdList.size() >= 1)){
						...
						//throw new BusinessException(new IllegalArgumentException("entity must exist").toString());
					...
					if(generatorDtoList != null){
						...
						if((conversionCtx.indexOf("NYCountyHL7GeneratorContext") != -1) ||
							...
							fileType = "HL7";
						} else if((conversionCtx.indexOf("CAHL7GeneratorContext") != -1)){
								generatorCtx = "CAHL7GeneratorContext";
								fileType = "HL7";
						} else if((conversionCtx.indexOf("ILHL7GeneratorContext") != -1)){
							generatorCtx = "ILHL7GeneratorContext";
							fileType = "HL7";

						} else if((conversionCtx.indexOf("PAHL7GeneratorContext") != -1)){
							generatorCtx = "PAHL7GeneratorContext";
							fileType = "HL7";
						...
						generatorDto.setEastWestFlag(eastWestFlag); //used for sending program header
						...
						try {
							if(fileType.equals("HL7")) System.out.println(fileType);
...
/*				
				for(StateResultDto dto : noaddrDtoList){
					StateMasterDto stateMasterDto = new StateMasterDto();
					stateMasterDto.setStateAbbreviation(dto.getReportableState());
					stateMasterDto.setStatus("active");
					List<StateMasterDto> stdList = asrBo.getStateMaster(stateMasterDto);
					log.warn("createResults(): stdList: " + (stdList == null ? "NULL" : stdList.toString()));
					if(stdList.size() == 1){
					...
					if(generatorDtoList != null){
						...
						//pMap.put("statePk", stateMasterDto.getStateMasterPk());
						//pMap.put("entityType", "Abnormal");
						//List<GeneratorFieldDto> selectedGdtoList = generatorBo.getGeneratorFieldTypeByStateAbbrev(pMap);
						//String fileType = null;
						//if((selectedGdtoList != null) && (selectedGdtoList.size() > 0)){
						//	fileType = selectedGdtoList.get(0).getGeneratorFieldType();
						//}
