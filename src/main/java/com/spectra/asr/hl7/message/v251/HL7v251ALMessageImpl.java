...
@Slf4j
public class HL7v251ALMessageImpl implements HL7Message {
	//private Logger log = Logger.getLogger(HL7v251ALMessageImpl.class);
...
//		list1.stream().filter(t ->  t % 2 == 0).forEach(t -> System.out.println("print Even: " + t));
		...
		if(patientRecord != null){
			...
			try {
				...
				String[] recApplication = fileReceivingApplication.split("\\^");

				if(recApplication.length > 1){
					msh.getReceivingApplication().getNamespaceID().setValue(recApplication[0]);
					msh.getReceivingApplication().getUniversalID().setValue(recApplication[1]);
					msh.getReceivingApplication().getUniversalIDType().setValue(recApplication[2]);
				} else {
				...
				String[] recFacility = fileReceivingFacility.split("\\^");
				if(recFacility.length > 1){
					msh.getReceivingFacility().getNamespaceID().setValue(recFacility[0]);
					msh.getReceivingFacility().getUniversalID().setValue(recFacility[1]);
					msh.getReceivingFacility().getUniversalIDType().setValue(recFacility[2]);
				} else {
				...
				//pid.getPid3_PatientIdentifierList(0).getCx2_CheckDigit().setValue("12345");
				//pid.getPid3_PatientIdentifierList(0).getCx3_CheckDigitScheme().setValue("12345");
				pid.getPid3_PatientIdentifierList(0).getCx4_AssigningAuthority().getNamespaceID().setValue(mshFileSendingFacility);
				pid.getPid3_PatientIdentifierList(0).getCx4_AssigningAuthority().getUniversalID().setValue(mshSendingFacilityId);
				// Change to ISO
				//pid.getPid3_PatientIdentifierList(0).getCx4_AssigningAuthority().getUniversalIDType().setValue(sendingFacilityIdType);

				// ** update to CLIA **
				pid.getPid3_PatientIdentifierList(0).getCx4_AssigningAuthority().getUniversalIDType().setValue("CLIA");

				pid.getPid3_PatientIdentifierList(0).getCx5_IdentifierTypeCode().setValue("PI");



				//pid.getPid3_PatientIdentifierList(0).getCx6_AssigningFacility().getNamespaceID().setValue("6");
				//pid.getPid3_PatientIdentifierList(0).getCx7_EffectiveDate().setValue("20200702");
				//pid.getPid3_PatientIdentifierList(0).getCx8_ExpirationDate().setValue("20200702");
				//pid.getPid3_PatientIdentifierList(0).getCx9_AssigningJurisdiction().getIdentifier().setValue("\\^");
...
//								}
								...
								//sn.getSeparatorSuffix().setValue(".");
//								if(obxRecord.getTestResultType().equals("NM")){
//									sn.getComparator().setValue("=");
//								} else {
//									sn.getComparator().setValue(">");
//								}
...
//							obx8_8.setValue("V1");
							...
							// ** update OBX 23.1
							obx.getPerformingOrganizationName().getOrganizationName().setValue("Spectra East");
							...
							// ** update OBX 23.10 organization identifier
							obx.getPerformingOrganizationName().getOrganizationIdentifier().setValue("31D0961672");
...
//				OBR parentobr = orderObservation.getOBR();
//				log.warn("toHL7Dto(): obr: " + (obr == null ? "NULL" : parentobr.getName()));
//				parentobr.getSetIDOBR().setValue("1");
//
//				// OBR 2
//				parentobr.getPlacerOrderNumber().getEntityIdentifier().setValue(patientRecord.getAccessionNo());
//
//				// OBR 3
//				parentobr.getFillerOrderNumber().getEntityIdentifier().setValue(patientRecord.getOrderNumber()); //patientrecord.getOrder_number
//			parentobr.getFillerOrderNumber().getUniversalIDType().setValue(sendingFacilityIdType);
//
//
//				parentobr.getObr16_OrderingProvider(0).getAssigningAuthority().getNamespaceID().setValue("NPI");
//				parentobr.getObr16_OrderingProvider(0).getAssigningAuthority().getUniversalID().setValue(mshSendingFacilityId);
//				parentobr.getObr16_OrderingProvider(0).getAssigningAuthority().getUniversalIDType().setValue(sendingFacilityIdType);
//
//				parentobr.getObr16_OrderingProvider(0).getIdentifierTypeCode().setValue("NPI");
//				parentobr.getObr16_OrderingProvider(0).getNameTypeCode().setValue("L");
//
//
//
//				//OBR 4.1
//				parentobr.getObr4_UniversalServiceIdentifier().getCe1_Identifier().setValue(obxRec.getLoincCode());
//				//OBR 4.2
//				parentobr.getObr4_UniversalServiceIdentifier().getCe2_Text().setValue(obxRec.getLoincName());
//				//OBR 4.3
//				parentobr.getObr4_UniversalServiceIdentifier().getCe3_NameOfCodingSystem().setValue("LN");
//
//				parentobr.getObr4_UniversalServiceIdentifier().getCe4_AlternateIdentifier().setValue(obxRec.getCompoundTestCode()); //obxrecord.getCompound_test_code
//				parentobr.getObr4_UniversalServiceIdentifier().getCe5_AlternateText().setValue(obxRec.getSeqTestName()); //obxrecord.getSeq_test_name
//				parentobr.getObr4_UniversalServiceIdentifier().getCe6_NameOfAlternateCodingSystem().setValue("L");
//
//				// Format collection date and time format
//				parentobr.getObservationDateTime().getTime().setValue(patientRecord.getCollectionDate()); //patientrecord.getCollection_date
//				parentobr.getRelevantClinicalInformation().setValue("Recurring patient"); // NEED REQUIREMENT FROM QA
//				parentobr.getSpecimenActionCode().setValue("G");
//
//				// OBR_15 Specimen source delivered in SPM
//				//parentobr.getSpecimenSource().getSpecimenSourceNameOrCode().getIdentifier().setValue(patientRecord.getSpecimenSource()); //patientrecord.getSpecimen_source
//				parentobr.getOrderingProvider(0).getIDNumber().setValue(patientRecord.getProviderId()); //patientrecord.getProvider_id
//				//String orderingPhyName = patientRecord.getOrderingPhyName();
//				//log.warn("toHL7Dto(): orderingPhyName: " + (orderingPhyName == null ? "NULL" : orderingPhyName));
//				if(StringUtils.contains(orderingPhyName, "^")){
//					String[] phyNameArray = StringUtils.split(orderingPhyName, "\\^");
...
//						//log.warn("toHL7Dto(): nameArray[0]: " + (phyNameArray[0] == null ? "NULL" : phyNameArray[0]));
//						//log.warn("toHL7Dto(): nameArray[1]: " + (phyNameArray[1] == null ? "NULL" : phyNameArray[1]));
//						parentobr.getOrderingProvider(0).getFamilyName().getSurname().setValue(phyNameArray[0]); //patientrecord.getOrdering_phy_name
//						parentobr.getOrderingProvider(0).getGivenName().setValue(phyNameArray[1]); //patientrecord.getOrdering_phy_name
//					}
//				}else{
//					parentobr.getOrderingProvider(0).getFamilyName().getSurname().setValue(orderingPhyName); //last name
//				}
//				parentobr.getOrderCallbackPhoneNumber(0).getAreaCityCode().setValue(patientRecord.getFacilityAreaCode()); //patientrecord.getFacility_area_code
//
//				parentobr.getOrderCallbackPhoneNumber(0).getTelecommunicationEquipmentType().setValue("PH");
//				parentobr.getOrderCallbackPhoneNumber(0).getLocalNumber().setValue(patientRecord.getFacilityPhoneNumber());
//				parentobr.getOrderCallbackPhoneNumber(0).getTelecommunicationUseCode().setValue("WPN");
//				parentobr.getOrderCallbackPhoneNumber(0).getCountryCode().setValue("1");
//
//				parentobr.getSpecimenReceivedDateTime().getTime().setValue(patientRecord.getSpecimenRecDateformat());
//
//				parentobr.getResultsRptStatusChngDateTime().getTime().setValue(patientRecord.getResRprtStatusChngDtTime()); //patientrecord.getRes_rprt_status_chng_dt_time
//				parentobr.getDiagnosticServSectID().setValue("LAB"); // NEED REQUIREMENT FROM QA
//				parentobr.getResultStatus().setValue(patientRecord.getRequisitionStatus()); //patientrecord.getRequisition_status
//
//
//
//				parentobr.getParent(); // NEED REQUIREMENT FROM QA
//				parentobr.getReasonForStudy(); // NEED REQUIREMENT FROM QA
				...
				// ** update SPM 2
				spm.getSpecimenID().getPlacerAssignedIdentifier().getNamespaceID().setValue("Spectra Labs");
				...
				spm.getSpecimenID().getFillerAssignedIdentifier().getNamespaceID().setValue("Spectra Labs");
				...
				// SPM 8
				spm.getSpecimenSourceSite().getIdentifier().setValue(obxRec.getSpecimenSourceIdentifier());
				spm.getSpecimenSourceSite().getText().setValue(obxRec.getSpecimenSourceText());
				spm.getSpecimenSourceSite().getNameOfCodingSystem().setValue(obxRec.getSourceTableSPM());
				spm.getSpecimenType().getCwe7_CodingSystemVersionID().setValue("1.0");
