...
@Slf4j
public class HL7v251NMMessageImpl implements HL7Message {
	//private Logger log = Logger.getLogger(HL7v251NMMessageImpl.class);
...
//		}
		...
		System.out.println("NM: " + patientRecord.getPatientName());
		...
		if(patientRecord != null){
			...
			if(performingLabId.startsWith("SH")){
				info = "SH";
			} else {
				info = "SE";
			}
			String infoSpectraAddress = (String)paramMap.get("infoSpectraAddress" + info);
			String infoSpectraAddress2 = (String)paramMap.get("infoSpectraAddress2" + info);
			String infoSpectraCity = (String)paramMap.get("infoSpectraCity" + info);
			String infoSpectraState = (String)paramMap.get("infoSpectraState" + info);
			String infoSpectraZip = (String)paramMap.get("infoSpectraZip" + info);
			String medicalDirector = (String)paramMap.get("medicalDirector" + info);
			...
			try {
				...
				ST pid_race_10_7 = new ST(oru);

				pid_race_10_7.setValue("2.5.1");
				...
				//pid.getRace(0).getExtraComponents().getComponent(7).setData(pid_race_10_7);
				...
				pid.getRace(0).getExtraComponents().getComponent(0).setData(pid_race_10_7);
...
//				orc.getPlacerGroupNumber().getNamespaceID().setValue("");
//				orc.getPlacerGroupNumber().getUniversalID().setValue("RequisitionID");
//				orc.getPlacerGroupNumber().getUniversalIDType().setValue("L");
...
*/					
					...
					for(int i = 0; i < obxList.size(); i++){
						...
						if((obxRecord.getSubTestCode() != null) &&
							...
							if(obxRecord.getTestResultType().equals("ST")){
							...
							obx.getObx15_ProducerSReference().getCe3_NameOfCodingSystem().setValue(sendingFacilityIdType);
							// OBX-17 OrderMethod
...
//								ce.getCe6_NameOfAlternateCodingSystem().setValue(values[5]);
							...
							if(testResultType.equals("NM") || testResultType.equals("SN")) {
								if(obxRecord.getUnits().equals("")) {
									obx.getUnits().getCe1_Identifier().setValue("index");
								} else {
							...
							// OBX 23.6
							obx.getPerformingOrganizationName().getAssigningAuthority().getNamespaceID().setValue(fileSendingFacility);
...
*/
				...
				spm.getSpecimenID().getPlacerAssignedIdentifier().getNamespaceID().setValue(fileSendingFacility);
				spm.getSpecimenID().getPlacerAssignedIdentifier().getUniversalID().setValue(sendingFacilityId);
				...
				spm.getSpecimenID().getFillerAssignedIdentifier().getNamespaceID().setValue(fileSendingFacility);
				spm.getSpecimenID().getFillerAssignedIdentifier().getUniversalID().setValue(sendingFacilityId);
