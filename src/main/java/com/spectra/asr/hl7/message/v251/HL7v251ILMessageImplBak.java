...
@Slf4j
public class HL7v251ILMessageImplBak implements HL7Message {
	//private Logger log = Logger.getLogger(HL7v251ILMessageImpl.class);

	private final static String V_LOINCCODE = "43409-2";
	private final static String V_LOINCNAME = "Bacteria identified:Prid:Pt:Isolate:Nom:Culture";
	private final static String V_SNOMEDCODE = "6265002";
	private final static String V_ORDERCODE = "750";
	private final static String V_ORDERNAME = "Culture, Blood";
	private final static String V_ORGANISM = "";
...
//		}
		...
		if(patientRecord != null){
			...
			if(performingLabId.indexOf("SW") != -1){
				//sendout west address
				fileSendingFacility = ApplicationProperties.getProperty("spectra.west.fhs.4");
				sendingApplication = ApplicationProperties.getProperty("spectra.west.msh.3");
				sendingFacilityId = ApplicationProperties.getProperty("spectra.west.msh.4.2");
			}else if(performingLabId.indexOf("SE") != -1){
				//east address
				fileSendingFacility = ApplicationProperties.getProperty("spectra.east.fhs.4");
				sendingApplication = ApplicationProperties.getProperty("spectra.east.msh.3");
				sendingFacilityId = ApplicationProperties.getProperty("spectra.east.msh.4.2");
			...
			try {
				...
				msh.getSendingApplication().getHd1_NamespaceID().setValue("Spectra East Test");
				//Spectra OID
				msh.getSendingApplication().getHd2_UniversalID().setValue("31D0961672");
				// sendingfacilityidtype should be changed to ISO
				sendingFacilityIdType = "CLIA";
				msh.getSendingApplication().getHd3_UniversalIDType().setValue(sendingFacilityIdType);
				...
				if ((fileReceivingApplication != null) || (fileReceivingApplication.length() > 0)) {
					String ra[] = fileReceivingApplication.split("\\^");

					String entityId = (ra[0] == null ? "" : ra[0]);
					String namespaceId = (ra[1] == null ? "" : ra[1]);
					String universalId = (ra[2] == null ? "" : ra[2]);

					msh.getReceivingApplication().getNamespaceID().setValue(entityId.trim());
					//msh.getReceivingApplication().getUniversalID().setValue(namespaceId.trim());
					//msh.getReceivingApplication().getUniversalIDType().setValue(universalId.trim());

				}

				if ((fileReceivingFacility != null) || (fileReceivingFacility.length() > 0)) {
					String ra[] = fileReceivingFacility.split("\\^");

					String entityId = (ra[0] == null ? "" : ra[0]);
					String namespaceId = (ra[1] == null ? "" : ra[1]);
					String universalId = (ra[2] == null ? "" : ra[2]);

					msh.getReceivingFacility().getNamespaceID().setValue(entityId.trim());
					//msh.getReceivingFacility().getUniversalID().setValue(namespaceId.trim());
					//msh.getReceivingFacility().getUniversalIDType().setValue(universalId);
...
//
				...
				pid.getPatientAddress(0).getXad9_CountyParishCode().setValue("");
				...
				pid.getEthnicGroup(0).getIdentifier().setValue(patientRecord.getHL7EthnicGroupCode());
				pid.getEthnicGroup(0).getText().setValue(patientRecord.getEthnicGroup());
				// Ethnic group source table HL70396
				...
				System.out.println("************ Printing OBX List **************");
...
//				orc.getPlacerGroupNumber().getUniversalIDType().setValue(sendingFacilityIdType);
				...
				orc.getOrc12_OrderingProvider(0).getAssigningAuthority().getUniversalID().setValue(sendingFacilityId);
				...
				//OBR obr2 = patientResult.getORDER_OBSERVATION().getOBR();
				//obr2.getSetIDOBR().setValue("2");
...
//				}
				...
				obr.getObservationDateTime().getTime().setValue(obxRec.getRealeaseDateTimeTZ()); //patientrecord.getCollection_date
				obr.getRelevantClinicalInformation().setValue("NG"); // NEED REQUIREMENT FROM QA
...
*/					
					...
					for(int i = 0; i < obxList.size(); i++){
						...
						//ORU_R01_OBSERVATION observation2 = orderObservation.getOBSERVATION(1);
						...
						if((obxRecord.getSubTestCode() != null) &&
							...
							if (obxRecord.getMicroOrganismName() != null) testResultType = "CE";
...
//							obx.getObx15_ProducerSReference().getCe3_NameOfCodingSystem().setValue(sendingFacilityIdType + "0396");
							...
							if ((obxRecord.getSubTestCode() != null) && (obxRecord.getSubTestCode().length() > 0)) {
								...
								//V_ORDERCODE
								builder.append(obxRecord.getCompoundTestCode()).append("-1-001");
								obx.getObservationIdentifier().getAlternateIdentifier().setValue(builder.toString());
								// V_ORDERNAME
								obx.getObservationIdentifier().getAlternateText().setValue(obxRecord.getSeqResultName());
...
//							obx8_8.setValue("V1");
							...
							obx.getDateTimeOfTheObservation().getTime().setValue(obxRecord.getRealeaseDateTimeTZ()); //patientrecord.getCollection_date
							...
							// ** update 23.6.2 - 31D0961672
							obx.getPerformingOrganizationName().getAssigningAuthority().getUniversalID().setValue("31D0961672");
								...
								for(int j = 0; j < obxTextNumResultArray.length; j++) {
									if(!(obxTextNumResultArray[j].trim().equalsIgnoreCase(""))) {
										...
										if((obxRecord.getLoincCode() != null) && (obxRecord.getLoincCode().length() > 0)){
											obx.getObservationIdentifier().getIdentifier().setValue(obxRecord.getILLoincCode());
											obx.getObservationIdentifier().getText().setValue(obxRecord.getILLoincName());
											obx.getObservationIdentifier().getNameOfCodingSystem().setValue("LN");
										}
										if((obxRecord.getSubTestCode() != null) && (obxRecord.getSubTestCode().length() > 0)){
											StringBuilder builder = new StringBuilder();
...
*/
							...
							ctr--;
//							obr = orderObservation.getOBR();
//							obr.insertOrderingProvider(1);
//							patientResult.getORDER_OBSERVATION().insertNTE(0);
						}else{
//							obr = orderObservation.getOBR();
//							obr.insertOrderingProvider(1);
//							orderObservation.insertOBSERVATION(1);
//							patientResult.getORDER_OBSERVATION().insertNTE(0);
//							obr = patientResult.getORDER_OBSERVATION(1).getOBR();
////							obr.getSetIDOBR().setValue("2");
////							newobx = observation.getOBX();
////							newobx.getSetIDOBX().setValue("1");
//							ctr -= 1;
						}//if
				...
				//orderObservation.insertOBSERVATION(obr);
				//obr = patientResult.getORDER_OBSERVATION(1).getOBR();
				//obr.getSetIDOBR().setValue("2");
				//obx = orderObservation.getOBSERVATION(1).getOBX();
				//obx.getSetIDOBX().setValue("1");

				System.out.println(obr.getParent().isRepeating("OBR"));
				obr.insertOrderingProvider(1);
				...
				// SPM 4
				spm.getSpecimenType().getIdentifier().setValue(obxRec.getSpecimenSourceSnomed().get(0));
				spm.getSpecimenType().getText().setValue(obxRec.getSpecimenSourceSnomed().get(1));
				spm.getSpecimenType().getNameOfCodingSystem().setValue(obxRec.getSpecimenSourceSnomed().get(2));
				...
				spm.getSpecimenSourceSite().getIdentifier().setValue(obxRec.getHL7Table().get(0));
				spm.getSpecimenSourceSite().getText().setValue(obxRec.getHL7Table().get(1));
				spm.getSpecimenSourceSite().getNameOfCodingSystem().setValue(obxRec.getHL7Table().get(2));
