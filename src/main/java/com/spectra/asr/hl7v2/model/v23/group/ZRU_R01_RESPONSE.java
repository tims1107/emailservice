package com.spectra.asr.hl7v2.model.v23.group;
...
public class ZRU_R01_RESPONSE extends AbstractGroup {
	//private Logger log = Logger.getLogger(ZRU_R01_RESPONSE.class);
    ...
    private void init(ModelClassFactory factory) {
        try {
           log.warn("init(): factory: " + (factory == null ? "NULL" : factory.toString()));
        	this.add(ORU_R01_PATIENT.class, false, false, false);
        	log.warn("init(): added : ORU_R01_PATIENT");
           this.add(ZRU_R01_ORDER_OBSERVATION.class, true, true, false);
           log.warn("init(): added : ZRU_R01_ORDER_OBSERVATION");
        ...
        if(names != null){
        	for(String name : names){
        		log.warn("init(): name: " + (name == null ? "NULL" : name));
        	}
        }
        
        ORU_R01_PATIENT pat = this.getPATIENT();
        log.warn("init(): pat: " + (pat == null ? "NULL" : pat.toString()));
     }
     ...
     public ORU_R01_PATIENT getPATIENT() { 
        ...
        log.warn("getPATIENT(): retVal: " + (retVal == null ? "NULL" : retVal.toString()));
        return retVal;
     }
