package com.spectra.asr.distributor.strategy.local;
...
@Slf4j
public class LocalDistributorStrategyImpl implements DistributorStrategy<Boolean> {
	//private Logger log = Logger.getLogger(LocalDistributorStrategyImpl.class);
			...
			try{
				if(generatorBo != null){
					...
					if(distributorBo != null){
						...
						String localRunMode = distItemMap.get("local.run.mode");
						String localIp = distItemMap.get("local.ip");
						String localUser = distItemMap.get("local.user");
						String localPwd = distItemMap.get("local.pwd");
							...
							*/
							...
							if(writeBy != null){
								...
								if((writeBy.equalsIgnoreCase("state")) ||(writeBy.equalsIgnoreCase("county"))){
									archivePath = distItemMap.get("folder.state.archive");
									archivePath += dt;
									...
									dropPath = distItemMap.get("folder.state.drop");
									dropPath += dt;
									//StringUtils.replace(archivePath, "\\", "\\\\");
									log.warn("distribute(): dropPath: " + (dropPath == null ? "NULL" : dropPath));
									dropDir = new File(dropPath);
								}else if(writeBy.equalsIgnoreCase("state_micro")){
									archivePath = distItemMap.get("folder.state.archive");
									int idxDSR = archivePath.indexOf("DailyStateReporting");
									archivePath = archivePath.substring(0, (idxDSR + "DailyStateReporting".length())) + "\\Micro\\";
									archivePath += dt;
									//StringUtils.replace(archivePath, "\\", "\\\\");
									log.warn("distribute(): archivePath: " + (archivePath == null ? "NULL" : archivePath));
									archiveDir	= new File(archivePath);						
									...
									dropPath = distItemMap.get("folder.state.drop");
									idxDSR = dropPath.indexOf("DailyStateReporting");
									dropPath = dropPath.substring(0, (idxDSR + "DailyStateReporting".length())) + "\\Micro\\";
									dropPath += dt;
									//StringUtils.replace(archivePath, "\\", "\\\\");
									log.warn("distribute(): dropPath: " + (dropPath == null ? "NULL" : dropPath));
									dropDir = new File(dropPath);
