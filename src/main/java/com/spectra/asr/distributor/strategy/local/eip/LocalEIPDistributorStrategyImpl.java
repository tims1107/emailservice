package com.spectra.asr.distributor.strategy.local.eip;
...
@Slf4j
public class LocalEIPDistributorStrategyImpl implements DistributorStrategy<Boolean> {
	//private Logger log = Logger.getLogger(LocalEIPDistributorStrategyImpl.class);
			...
			try{
				if(generatorBo != null){
					...
					if(distributorBo != null){
						...
						//distributorItemDto.setDistributorFk(null);
						...
						String netStart = ApplicationProperties.getProperty("net.start.local");
						String netEnd = ApplicationProperties.getProperty("net.end.local");
						String localRunMode = distItemMap.get("eip.local.run.mode");
						String localIp = distItemMap.get("eip.local.ip");
						String localUser = distItemMap.get("eip.local.user");
						String localPwd = distItemMap.get("eip.local.pwd");
						...
						log.warn("distribute(): localRunMode: " + (localRunMode == null ? "NULL" : localRunMode));
						log.warn("distribute(): localIp: " + (localIp == null ? "NULL" : localIp));
						log.warn("distribute(): localUser: " + (localUser == null ? "NULL" : localUser));
						log.warn("distribute(): localPwd: " + (localPwd == null ? "NULL" : localPwd));
						...
						if(filePath != null){
							...
							*/
							...
							String writeBy = this.generatorDto.getWriteBy();
							if(writeBy != null){
								StringBuilder filePathBuilder = new StringBuilder();
								//filePathBuilder.append(filePath).append(this.generatorDto.getWriteBy()).append("/").append(dt).append("/").append(fName);
								//filePathBuilder.append(filePath).append(writeBy).append("/").append(dt).append("/");
								filePathBuilder.append(filePath).append(writeBy).append("/").append(dtt).append("/");
								...
								String[] stateTargetArray = null;
								
								if((writeBy.equalsIgnoreCase("state")) || (writeBy.equalsIgnoreCase("county"))){
									if(distributorStateTarget != null){
										stateTargetArray = distributorStateTarget.split(",");
									}
									archivePath = distItemMap.get("eip.folder.state.archive");
									archivePath = MessageFormat.format(archivePath, new Object[]{ dtt, });
									log.warn("distribute(): archivePath: " + (archivePath == null ? "NULL" : archivePath));
									archiveDir	= new File(archivePath);
									
									dropPath = distItemMap.get("eip.folder.state.drop");
									int idxEip = dropPath.indexOf("EIP");
									statePath = dropPath.substring(0, (idxEip + "EIP".length())) + "\\";
									dropPath = MessageFormat.format(dropPath, new Object[]{ dtt, });
									log.warn("distribute(): dropPath: " + (dropPath == null ? "NULL" : dropPath));
									dropDir = new File(dropPath);
								}else if(writeBy.equalsIgnoreCase("state_all")){
									archivePath = distItemMap.get("eip.folder.all.state.archive");
									archivePath = MessageFormat.format(archivePath, new Object[]{ dtt, });
									log.warn("distribute(): archivePath: " + (archivePath == null ? "NULL" : archivePath));
									archiveDir	= new File(archivePath);
									
									dropPath = distItemMap.get("eip.folder.all.state.drop");
									dropPath = MessageFormat.format(dropPath, new Object[]{ dtt, });
									log.warn("distribute(): dropPath: " + (dropPath == null ? "NULL" : dropPath));
									dropDir = new File(dropPath);
								}
								...
								//archivePath += dt;
								archivePath = MessageFormat.format(archivePath, new Object[]{ dt, });
								//StringUtils.replace(archivePath, "\\", "\\\\");
								log.warn("distribute(): archivePath: " + (archivePath == null ? "NULL" : archivePath));
								File archiveDir	= new File(archivePath);	
								
								String allStateArchivePath = distItemMap.get("folder.all.state.archive");
								allStateArchivePath = MessageFormat.format(allStateArchivePath, new Object[]{ dt, });
								//StringUtils.replace(archivePath, "\\", "\\\\");
								log.warn("distribute(): allStateArchivePath: " + (allStateArchivePath == null ? "NULL" : allStateArchivePath));
								File allStateArchiveDir	= new File(allStateArchivePath);	
								
								String dropPath = distItemMap.get("folder.state.drop");
								//dropPath += dt;
								dropPath = MessageFormat.format(dropPath, new Object[]{ dt, });
								//StringUtils.replace(archivePath, "\\", "\\\\");
								log.warn("distribute(): dropPath: " + (dropPath == null ? "NULL" : dropPath));
								File dropDir = new File(dropPath);
								
								String allStateDropPath = distItemMap.get("folder.all.state.drop");
								//dropPath += dt;
								allStateDropPath = MessageFormat.format(allStateDropPath, new Object[]{ dt, });
								//StringUtils.replace(archivePath, "\\", "\\\\");
								log.warn("distribute(): allStateDropPath: " + (allStateDropPath == null ? "NULL" : allStateDropPath));
								File allStateDropDir = new File(allStateDropPath);
								*/
								...
								if(localDir.exists()){
									...
									if(stateTargetArray != null){
										//wroteState = AsrFileWriter.copyFilesToDir(localDir, statePath, stateTargetArray, this.generatorDto.getStateAbbreviation());
										wroteState = AsrFileWriter.copyFilesToDir(localDir, statePath, stateTargetArray, this.generatorDto.getStateAbbreviation(), dtt);
									}
									...
									//distributed = new Boolean(wroteState && wroteArchive && wroteDrop);
									distributed = new Boolean(wroteState || wroteArchive || wroteDrop);
